Platform.sh PHP 7.2 Example
===========================

This example exists to test this plugin's implementation of Platform.sh's `php:7.2` service.

* [`php:7.2`](https://docs.platform.sh/configuration/services/php.html)
...
docker ps --filter label=com.docker.compose.project=landoplatformshphp72 | grep docker.registry.platform.sh/php-7.2 | grep landoplatformshphp72_vanilla_1
docker ps --filter label=com.docker.compose.project=landoplatformshphp72 | grep docker.registry.platform.sh/php-7.2 | grep landoplatformshphp72_advanced_1
...
lando ssh -s vanilla -c "php --version" | grep 7.2.
lando ssh -s advanced -c "php --version" | grep 7.2.

# Should serve php files
lando ssh -s vanilla -c "curl localhost:80" | grep  "PHP Version 7.2."
lando ssh -s advanced -c "curl localhost:80" | grep  "PHP Version 7.2."
