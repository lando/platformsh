Platform.sh PHP 8.0 Example
===========================

This example exists to test this plugin's implementation of Platform.sh's `php:8.0` service.

* [`php:8.0`](https://docs.platform.sh/configuration/services/php.html)
...
docker ps --filter label=com.docker.compose.project=landoplatformshphp80 | grep docker.registry.platform.sh/php-8.0 | grep landoplatformshphp80_vanilla_1
docker ps --filter label=com.docker.compose.project=landoplatformshphp80 | grep docker.registry.platform.sh/php-8.0 | grep landoplatformshphp80_advanced_1
...
lando ssh -s vanilla -c "php --version" | grep 8.0.
lando ssh -s advanced -c "php --version" | grep 8.0.

# Should serve php files
lando ssh -s vanilla -c "curl localhost:80" | grep  "PHP Version 8.0."
lando ssh -s advanced -c "curl localhost:80" | grep  "PHP Version 8.0."
